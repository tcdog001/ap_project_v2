...
mac80211_generate_mac() {
	...
	[ "$wifi" = "wifi1" ] && mac_num=1;
	[ "$wifi" = "wifi0" ] && mac_num=0;

	local ref="$(mac80211_generate_mac_autelan $mac_num $macaddr $(cat /sys/class/net/${wifi}/broadcast))"
	local mask="$(cat /sys/class/net/${wifi}/broadcast)"
...
#add end by autelan20150326

find_qcawifi_phy() {
	local device="$1"
	
#       modify by autelan20150326
	local macaddr="$(config_get "$device" macaddr | tr 'A-Z' 'a-z')"
#	local macaddr="$(mac80211_generate_mac $device)"
	...
	[ -z "$macaddr" ] && {
#		config_set "$device" macaddr "$(mac80211_generate_mac $device)"
...
#	config_get setHwaddr "$device" setHwaddr
	local setHwaddr=$(mac80211_generate_mac "$device")
...
config wifi-device  wifi$devidx
	option type	qcawifi
	option channel	auto
	option macaddr	$(cat /sys/class/${dev}/address)
